...
struct Ini_node {
    ...
    uint32_t id;
    uint32_t max_users_per_session;
...
struct Ini_file {
    ...
    std::vector<std::unique_ptr<Ini_node>> nodes;

    Ini_file(const std::string& filename);

    Ini_node* get_master() const;
    Ini_node* get_me() const;
    void get_slaves(std::vector<Ini_node*>& slaves) const;
