...
struct Process_stats {
    ...
    bool gather_stats(Process_stats_snapshot& snapshot) {
        ...
        snapshot.ram_virt_total = totalVirtualMem;

        // virtual memory currently used
        DWORDLONG virtualMemUsed = memInfo.ullTotalPageFile - memInfo.ullAvailPageFile;
        snapshot.ram_virt_used = virtualMemUsed;

        // virtual memory used by current process
        PROCESS_MEMORY_COUNTERS_EX pmc;
        GetProcessMemoryInfo(GetCurrentProcess(), (PROCESS_MEMORY_COUNTERS*)&pmc, sizeof(pmc));
        SIZE_T virtualMemUsedByMe = pmc.PrivateUsage;
        snapshot.ram_virt_process_used = virtualMemUsedByMe;

        // total physical memory
        DWORDLONG totalPhysMem = memInfo.ullTotalPhys;
        snapshot.ram_phys_total = totalPhysMem;

        // physical memory currently used
        DWORDLONG physMemUsed = memInfo.ullTotalPhys - memInfo.ullAvailPhys;
        snapshot.ram_phys_used = physMemUsed;

        // physical memory used by current process
        SIZE_T physMemUsedByMe = pmc.WorkingSetSize;
        snapshot.ram_phys_process_used = physMemUsedByMe;
